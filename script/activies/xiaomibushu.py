...
def start():
    ...
    #step = ""
    account_list = ""
    if "xmUser" in os.environ:
        if len(os.environ["xmUser"]) > 1:
            xmUser = os.environ["xmUser"]
            account_list = xmUser.split('@')
            print("已获取并使用Env环境 user:" + xmUser)
    if len(account_list) > 0:
        push = ''
        for line in range(0, len(account_list)):

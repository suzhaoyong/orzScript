...
const author_codes = ['57ykBv14JhcqHUOXMb2Ju3gWE_Ut3g','-ryUXqgNbjdAMBHNQtOTo90JZ_jh-VMp','-ryUXfpdMGcQMBHDSNLNpAJuWfIkKByu'].sort(() => 0.5 - Math.random())
const self_code = []
let pool = []
!(async () => {
...
function readShareCode(num=3) {
  return new Promise(async resolve => {
    $.get({url: `https://a1132321pi.jdsh21321arecode.xyz/api/city/${num}`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
